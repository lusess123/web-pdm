...
> 本文保留了项目早期关于 Redux、MobX、Recoil（历史原文写作 Recoli）、MobX State Tree 和 boxer 设想的完整推导。相关库的现状可能已经变化，但“用关系图表达复杂状态”的设计思路仍然是 web-pdm 的产品基础。
...
boxer 和一键代码生成属于项目早期构想，并不是当前公开 API 的承诺。当前 web-pdm 更专注于稳定地接收模型元数据、展示显式关系，并通过 `onModelDetail`、`onReload`、`onIgnoreEdge` 等事件融入现有产品。

这仍然遵循原文的核心判断：模型元数据应当是事实来源，ER 图是它的可视化投影。宿主应用可以在这个稳定接口之上增加编辑规则、校验、代码生成或双向同步，而不需要直接依赖内部 G6 实例。
