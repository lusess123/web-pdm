...
export const createRootStore = (props) => {
  const { onIgnoreEdge, onModelDetail, ...serializableSys } = props.sys ?? {};
  const rootStore = createStore({
    ...props,
    sys: serializableSys,
  });
  rootStore.setOnReload(props.onReload);
  rootStore.setOnIntl(props.onIntl);
  rootStore.sys.setOnIgnoreEdge(onIgnoreEdge);
  rootStore.sys.setOnModelDetail(onModelDetail);
